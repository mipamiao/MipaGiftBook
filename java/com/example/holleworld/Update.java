...
public class Update {
    ...
    public Update(Data data,int index,IndirectClass ic){
        ...
        if(Index!=-1){
            pop_name.setText(DATA.LP.get(index).name);
            pop_money.setText(Person.ReMoneyTrans(DATA.LP.get(index).money));
            pop_memo.setText(DATA.LP.get(index).memo);
            pop_arc.setText(DATA.LP.get(index).arc);
        }
        ...
        OK.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(index==-1||index==DATA.LP.size()-1)Add();
                else Change();
                ((MainActivity)(IC.getActivity())).setPageCodeAndShow(
                        ((MainActivity)(IC.getActivity())).mid.PageCode
                );
                popwindow.dismiss();
            }
        });
    ...
    public void Change(){
        DATA.LP.get(Index).Change(pop_name.getText().toString(),
